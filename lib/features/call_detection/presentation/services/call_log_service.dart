...
class CallLogService {
  ...
  void startListening() {
    print('📞 CallLogService: Starting to listen for call events');

    // Subscribe to call events stream
    final callEventsStream = _ref.read(callEventsProvider.stream);
    
    _callEventsSubscription = callEventsStream.listen(
      (eitherCallEvent) {
        eitherCallEvent.fold(
          (failure) {
            print('❌ CallLogService: Error receiving call event: ${failure.message}');
          },
          (callEvent) {
      ...
      onError: (error) {
        print('❌ CallLogService: Stream error: $error');
      },
    );
  }
